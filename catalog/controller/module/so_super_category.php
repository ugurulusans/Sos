...
class SoSuperCategory extends \Opencart\System\Engine\Controller {	
	...
	public function index($setting) {
		$this->load->language('extension/so_entry/module/so_super_category','',$this->config->get('config_language'));
		...
		if (!defined ('OWL_CAROUSEL')){
			...
			$this->document->addStyle('extension/so_entry/catalog/view/javascript/so_super_category/css/owl.carousel.css');
			$this->document->addScript('extension/so_entry/catalog/view/javascript/so_super_category/js/owl.carousel.js');
		...
		if($listCategory_son != null){
			...
			if($is_ajax && isset($_POST['is_ajax_super_category']) && $_POST['is_ajax_super_category'] ){
				foreach ($filters as $filter) {
					...
					$aritles['count'] = $this->model_extension_so_entry_module_so_super_category->getTotalProducts_super_category($filter_data);
				...
				$html_data = $this->load->view('extension/so_entry/module/so_super_category/default_items', $data_template);		
		...
		if (!class_exists('Cache_Lite'))
		    require_once (DIR_EXTENSION . 'so_entry/system/library/so/super_category/Cache_Lite/Lite.php');
		...
		if ($use_cache){
			...
			if (!$_data) {
				/*$_data = $this->load->view('extension/so_entry/module/so_super_category/default', $data);*/
				$_data = $this->load->view('extension/so_entry/module/so_super_category/'.$data['store_layout'], $data);
			...
			/*return $this->load->view('extension/so_entry/module/so_super_category/default', $data);*/
			return $this->load->view('extension/so_entry/module/so_super_category/'.$data['store_layout'], $data);
		}
	}
	...
	private  function getCategoryson($setting){
		...
		foreach($setting['category'] as $category_item){
			$checkCategory = $this->model_extension_so_entry_module_so_super_category->checkCategory($category_item);
		...
		if($category_list != null){
			for($i=1; $i<=$setting['category_depth'];$i++){
				foreach ($setting['category'] as $categorys){
					...
					$categoryss = $this->model_extension_so_entry_module_so_super_category->getCategories_son($filter_data);
	...
	public function getProducts($category_id_list,$setting)	{
		...
		$cat['count'] = $this->model_extension_so_entry_module_so_super_category->getTotalProducts_super_category($filter_data);
		if ($cat['count'] > 0) {
			$products_arr = $this->model_extension_so_entry_module_so_super_category->getProducts_super_category($filter_data);
