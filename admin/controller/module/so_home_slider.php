...
class SoHomeSlider extends \Opencart\System\Engine\Controller {	
	...
	public function index() {
		...
		$this->load->language('extension/so_entry/module/so_home_slider','',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
	...
	// Get data
		$model = $this->model_extension_so_entry_module_so_home_slider; 
		$model->checkInstall();
	...
	{
		// Add js
		$this->document->addScript('../extension/so_entry/admin/view/javascript/so_home_slider/js/jquery-ui.min.js');
		$this->document->addStyle('../extension/so_entry/admin/view/javascript/so_home_slider/js/jquery-ui.min.css');
	}
	...
	// Delete Module
		if( isset($this->request->get['slide_id']) && isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			$this->model_extension_so_entry_module_so_home_slider->deleteSlide( $this->request->get['slide_id'] );
			$this->response->redirect($this->url->link('extension/so_entry/module/so_home_slider', 'user_token=' . $this->session->data['user_token'].'&tab=slide&module_id='.$this->request->get['module_id'], 'SSL'));
			...
			$this->model_extension_so_entry_module_so_home_slider->deleteAllSlide($this->request->get['module_id']);
			$this->response->redirect($this->url->link('extension/so_entry/module/so_home_slider', 'user_token=' . $this->session->data['user_token'], 'SSL'));
		}
	// Get Module Id New
		$moduleid_new= $this->model_extension_so_entry_module_so_home_slider->getModuleId(); // Get module id
		$module_id = '';	
		$slideid_new= $this->model_extension_so_entry_module_so_home_slider->getSlideId(); // Get slide id
		$slide_id = '';
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($tab_module == 'add-slide' || $tab == 'slide' && $tab_module != 'add-module'  )
			{
				...
				if (!isset($this->request->get['slide_id'])) {
					...
					$this->model_extension_so_entry_module_so_home_slider->addSlide($this->request->post);
				} else {
					$slide_id = $this->request->get['slide_id'];
					$this->request->post['moduleid'] = $this->request->get['module_id'];
					
					$this->model_extension_so_entry_module_so_home_slider->editSlide($this->request->get['slide_id'], $this->request->post);
				}
				
				if($action == "save_edit") {
					$this->response->redirect($this->url->link('extension/so_entry/module/so_home_slider', 'module_id='.$this->request->get['module_id'].'&tab=slide&slide_id='.$slide_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
				...
				if (!isset($this->request->get['module_id'])) {
					...
					$this->model_setting_module->addModule('so_entry.so_home_slider', $this->request->post);
				...
				if($action == "save_edit") {
					$this->response->redirect($this->url->link('extension/so_entry/module/so_home_slider', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
				}elseif($action == "save_new"){
					$this->response->redirect($this->url->link('extension/so_entry/module/so_home_slider', 'user_token=' . $this->session->data['user_token'], 'SSL'));
				}else{
					$this->response->redirect($this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'], 'SSL'));
		...
		if (isset($this->request->get['module_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST') || ($this->request->server['REQUEST_METHOD'] == 'POST' && !$this->validate() && isset($this->request->get['module_id'])) || ($this->request->server['REQUEST_METHOD'] == 'POST' && !$this->validate() && isset($this->request->get['slide_id']))) {
			...
			$data['action'] 				= $this->url->link('extension/so_entry/module/so_home_slider', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
			
			if(isset($this->request->get['slide_id'])){
				$data['action'] = $this->url->link('extension/so_entry/module/so_home_slider', 'user_token=' . $this->session->data['user_token'] . '&tab=slide&module_id='.$this->request->get['module_id'].'&slide_id=' . $this->request->get['slide_id'], 'SSL');
				
			}elseif(isset($this->request->get['tab']) && $this->request->get['tab'] == "slide")
			{
				$data['action'] = $this->url->link('extension/so_entry/module/so_home_slider', 'user_token=' . $this->session->data['user_token'] . '&tab=slide&module_id='.$this->request->get['module_id'], 'SSL');
			}
			...
			if(isset($this->request->get['slide_id'])){
				$data['slide_description'] = $this->model_extension_so_entry_module_so_home_slider->getSliderById($this->request->get['slide_id']);
			...
			$data['selectedid'] 	= 0;
			$data['action'] 		= $this->url->link('extension/so_entry/module/so_home_slider', 'user_token=' . $this->session->data['user_token'], 'SSL');
		...
		$data['moduletabs'] = $this->model_setting_module->getModulesByCode( 'so_entry.so_home_slider' );
		$data['link'] = $this->url->link('extension/so_entry/module/so_home_slider', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
		$data['linkremove'] = $this->url->link('extension/so_entry/module/so_home_slider&user_token=' . $this->session->data['user_token']);
		//--------------------------------Load Data -------------------------------------------
		...
		$this->response->setOutput($this->load->view('extension/so_entry/module/so_home_slider', $data));
	}
	...
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/so_entry/module/so_home_slider', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/so_entry/module/so_home_slider', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
		return $this->data['breadcrumbs'];
	}
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/so_entry/module/so_home_slider')) {
	...
	public function sortposition() {
		...
		if (isset($this->request->get['sortarray'])) {
			$result = $this->model_extension_so_entry_module_so_home_slider->updatePositionSlide($this->request->get['sortarray']);
			//var_dump($result);die();
		}
	}
