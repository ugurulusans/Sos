...
class SoMegaMenu extends \Opencart\System\Engine\Model {	
    ...
    public function generate_nestable_list($lang_id) {
        ...
        if (!isset($this->request->get['module_id'])) {
            $action = $this->url->link('extension/so_entry/module/so_megamenu', 'user_token=' . $this->session->data['user_token'], 'SSL');
        } else {
            $action = $this->url->link('extension/so_entry/module/so_megamenu', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
        }
	...
	public function duplicateModule($module_id,$import_module){
		...
		if($parent_menu){
			foreach ($parent_menu as $menu) {
				$dane = $this->model_extension_so_entry_module_so_megamenu->getMenu(intval($menu['id']));
				$dane['module_id'] = $module_id;
				$id_parent_add = $this->model_extension_so_entry_module_so_megamenu->addMenu($dane);
				$subcategories = $this->model_extension_so_entry_module_so_megamenu->getSubMenu(intval($menu['id']));
				if($subcategories){
				foreach ($subcategories as $result) {
					$data = array(
							...
							'list_categories' => (isset($result['content']['categories']['categories']) && $result['content']['categories']['categories']) ? $this->model_extension_so_entry_module_so_megamenu->getCategories(json_decode($result['content']['categories']['categories'], true)) : ''
						);
						$this->model_extension_so_entry_module_so_megamenu->addMenu($data);
					}
				}
			}
		}
	...
	public function duplicateMenu($id_duplicate){
		$dane = $this->model_extension_so_entry_module_so_megamenu->getMenu(intval($id_duplicate));
		$id_parent_add = $this->model_extension_so_entry_module_so_megamenu->addMenu($dane);
		$subcategories = $this->model_extension_so_entry_module_so_megamenu->getSubMenu(intval($id_duplicate));
		if($subcategories){
			foreach ($subcategories as $result) {
				$data = array(
					'parent_id' => $id_parent_add,
