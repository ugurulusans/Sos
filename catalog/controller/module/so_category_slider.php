...
class SoCategorySlider extends \Opencart\System\Engine\Controller {	
	public function index($setting) {
		static $module = 1;
		$this->load->language('extension/so_entry/module/so_category_slider','',$this->config->get('config_language'));
		...
		$this->document->addStyle('extension/so_entry/catalog/view/javascript/so_category_slider/css/slider.css');
			...
			if($setting['child_category']){
				for($i=1; $i<=$setting['category_depth'];$i++){
					foreach ($_catids as $categorys)	{
						...
						$categoryss = $this->model_extension_so_entry_module_so_category_slider->getCategories_son($filter_data);
		...
		if($setting['display_feature']){
			...
			if(count($setting['product_feature']) > 0){
				...
				$products_deals = $this->model_extension_so_entry_module_so_category_slider->getDeals($filter_data);
		...
		if (!class_exists('Cache_Lite'))
		    require_once (DIR_EXTENSION . 'so_entry/system/library/so/category_slider/Cache_Lite/Lite.php');
		...
		if ($use_cache){
			...
			if (!$_data) {
				$_data = $this->load->view('extension/so_entry/module/so_category_slider/default', $data);
			...
			return $this->load->view('extension/so_entry/module/so_category_slider/default', $data);
		}
	}
	public function getCategoryson($category_id, $setting)
	{
		...
		if($setting['child_category_cat'] ==1)
		{
			...
			$categoryss = $this->model_extension_so_entry_module_so_category_slider->getCategories_son_categories($filter_data);
		}
		return $categoryss;
	}
	public function getProducts($category_id_list,$setting)
	{
		...
		$cat['count'] = $this->model_extension_so_entry_module_so_category_slider->getTotalProducts_categories($filter_data);
		if ($cat['count'] > 0)
		{
			$products_arr = $this->model_extension_so_entry_module_so_category_slider->getProducts_categories($filter_data);
