...
class SoCategories extends \Opencart\System\Engine\Controller {	
	public function index($setting) {
		$this->load->language('extension/so_entry/module/so_categories','',$this->config->get('config_language'));
		...
		$folder_so_deal = DIR_TEMPLATE.$this->config->get('theme_default_directory').'/template/extension/so_entry/module/so_categories/';
		...
		if (!class_exists('Cache_Lite'))
		    require_once (DIR_EXTENSION . 'so_entry/system/library/so/categories/Cache_Lite/Lite.php');
		...
		if ($use_cache){
			...
			if (!$_data) {
				$_data = $this->load->view('extension/so_entry/module/so_categories/default', $data);
			...
			return $this->load->view('extension/so_entry/module/so_categories/default', $data);
		}
		
	}
	public function getCategoryson($category_id, $setting){
		$checkCategory = $this->model_extension_so_entry_module_so_categories->checkCategory($category_id);
		$categoryss = array();
		if(isset($checkCategory) && $checkCategory[0]['status'] == 1 && $checkCategory != null && $setting['child_category'] ==1){
			...
			$categoryss = $this->model_extension_so_entry_module_so_categories->getCategories_son_categories($filter_data);
		}
		return $categoryss;
	}
